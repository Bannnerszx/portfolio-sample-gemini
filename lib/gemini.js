...
// Optional egress proxy. The Generative Language API enforces caller location,
// and the deployment VM sits in a blocked region — every call 400s with
// FAILED_PRECONDITION before the model is reached. GEMINI_BASE_URL points at a
// Cloudflare Worker (see cloudflare/) whose egress is in a supported region.
// Unset locally, so dev talks to Google directly and this is a no-op.
function httpOptions() {
  const baseUrl = process.env.GEMINI_BASE_URL?.trim();
  if (!baseUrl) return undefined;
  const token = process.env.GEMINI_PROXY_TOKEN?.trim();
  return {
    baseUrl,
    // Shared secret so the Worker isn't an open relay to Google.
    ...(token ? { headers: { "x-proxy-token": token } } : {}),
  };
}
